...
def crear_botones(botones_config, share_text=None):
    ...
    # Botón SHARE por defecto (si existe share_text)
    if share_text:
...
async def menu_principal(update, context, edit=False, grupo_id=None):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⏰ Eliminar Mensaje", callback_data=f"menu_eliminar_{grupo_id}")],
...
async def menu_callback(update, context):
    ...
    if data.startswith("menu_eliminar_"):
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)
        tiempo = grupo_config.get('tiempo_eliminacion', 0)
        await query.edit_message_text(
            f"🗑️ *ELIMINAR MENSAJE*\n\n"
            f"Actual: {tiempo}s\n\n"
            f"Los mensajes se eliminarán después de este tiempo.\n"
            f"0 = No eliminar",
            parse_mode="Markdown",
            reply_markup=reply_markup
        )
...
async def handle_join_request(update, context):
    try:
        ...
        try:
            ...
            tiempo_elim = grupo_config.get('tiempo_eliminacion', 0)
            if tiempo_elim > 0:
...
def main():
    ...
    # Callbacks
    application.add_handler(CallbackQueryHandler(menu_callback, pattern="menu_|welcome_|media_|botones_|mensaje_|auto_|t_|fijar_|elim_|reset_|alert_|edit_|delete_|custom_"))
